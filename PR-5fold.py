...
def MyEnlarge(x0, y0, width, height, x1, y1, times, mean_fpr, mean_tpr, thickness=1, color = 'blue'):
    def MyFrame(x0, y0, width, height):
        ...
        plt.plot(xk, ykn, color='k', lnestyle=':', lw=1, alpha=1)  # 上

        return
    # 画虚线框
    width2 = times * width
    height2 = times * height
    MyFirame(x0, y0, width, height)
...
while counter0 < 5:
    ...
    RAPName = r'svm_prediction result' + str(counter0) + '.csv'
    RAPNameProb = r'svm_prediction result prob' + str(counter0) + '.csv'
...
plt.ylim([0.0, 1.0])
plt.xlim([0.0, 1.0])
#plt.title('Precision-Recall curve'.format(mean_RPs))
# 画网格
# plt.grid(linestyle='-')
# 画对角线
plt.plot([1, 0], [0, 1], color='navy', lw=2, linestyle='--')
plt.legend(bbox_to_anchor=(0.47, 0.4))
